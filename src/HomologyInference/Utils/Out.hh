...
#define ISM_INFO_wait(str) \
        {std::cout << HomologyInference::ANSI_FG_GREEN << str << HomologyInference::ANSI_RESET; \
        std::cout.flush();}

#define ISM_INFO(str) \
        {std::cout << HomologyInference::ANSI_FG_GREEN << str << HomologyInference::ANSI_RESET << std::endl; \
        std::cout.flush();}

#define ISM_HIGHLIGHT(str) \
        {std::cout << HomologyInference::ANSI_FG_GREEN << HomologyInference::ANSI_BG_MAGENTA << str << HomologyInference::ANSI_RESET << std::endl; \
        std::cout.flush();}

#define ISM_DEBUG_OUT(str) \
        {std::cout << HomologyInference::ANSI_FG_MAGENTA \
                   << "[DEBUG] " \
                   << str \
                   << HomologyInference::ANSI_RESET << std::endl; \
        std::cout.flush();}
...
#define ISM_WARNING(str) \
        {std::cout << HomologyInference::ANSI_FG_YELLOW \
                  << "[WARNING] " \
                  << str \
                  << HomologyInference::ANSI_RESET \
...
#define ISM_ERROR(str) \
        std::cout << HomologyInference::ANSI_FG_RED \
                  << "[ERROR] " \
                  << str \
                  << HomologyInference::ANSI_RESET \
                  << " (in function " << __FUNCTION__ << ":" << __LINE__ \
...
{ \
    auto A_eval = A; \
    for (int i = 0; i < (A_eval).cols(); ++i) \
    { \
        for (int j = 0; j < (A_eval).rows(); ++j) \
        { \
