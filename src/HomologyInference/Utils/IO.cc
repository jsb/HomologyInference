        ...
        const bool _silent)
{
    ...
    // Detect whether the mesh comes with texture coordinates.
    // Currently obj only. OpenMesh cannot detect this on its own?
    OpenMesh::IO::Options options;
    if (_path.extension() == ".obj" && obj_has_texture_coordinates(_path))
    {
        ...
        const bool _throw_if_not_exist)
{
    ...
    if (format == LandmarkFormat::Pinned)
        return read_landmarks_pinned(_path, _throw_if_not_exist);
...
std::vector<VH> read_landmarks_pinned(
        ...
        const bool _throw_if_not_exist)
{
    return read_element_handles<VH>(_path, _throw_if_not_exist);
}
        ...
        const bool _overwrite)
{
    const LandmarkFormat format = guess_landmark_format(_path);
    if (format == LandmarkFormat::Pinned)
        write_landmarks_pinned(_landmarks, _path, _overwrite);
    else if (format == LandmarkFormat::VTS)
...
void write_landmarks_pinned(
        ...
        const bool _overwrite)
{
    write_element_handles(_landmarks, _path, _overwrite);
}
...
TriMesh to_open_mesh(
        const pm::vertex_attribute<Vec3d>& pos)
{
    TriMesh mesh;

    for (auto v : pos.mesh().vertices())
        mesh.add_vertex(pos[v]);
    for (auto f : pos.mesh().faces())
        mesh.add_face(f.vertices().to_vector([] (auto v) { return VH(v.idx.value); }));

    return mesh;
}

TriMesh to_open_mesh(
        const pm::vertex_attribute<tg::pos3>& pos)
{
    TriMesh mesh;

    for (auto v : pos.mesh().vertices())
        mesh.add_vertex(to_vec3<double>(pos[v]));
    for (auto f : pos.mesh().faces())
        mesh.add_face(f.vertices().to_vector([] (auto v) { return VH(v.idx.value); }));
